...
def generate_ontology_turtle():
    ...
    lines.append("cosmo:maxConcentrationEU a owl:DatatypeProperty ; rdfs:domain cosmo:CosmeticIngredient ; rdfs:range xsd:decimal .")
    lines.append("cosmo:maxConcentrationUS a owl:DatatypeProperty ; rdfs:domain cosmo:CosmeticIngredient ; rdfs:range xsd:decimal .")
    lines.append("cosmo:maxConcentrationChina a owl:DatatypeProperty ; rdfs:domain cosmo:CosmeticIngredient ; rdfs:range xsd:decimal .")
    lines.append("cosmo:maxConcentrationJapan a owl:DatatypeProperty ; rdfs:domain cosmo:CosmeticIngredient ; rdfs:range xsd:decimal .")
...
def generate_shacl_turtle():
    ...
    lines.append('        sh:datatype xsd:decimal ;')
    ...
    # SPARQL-based SHACL constraint for concentration validation
    lines.append("cosmo:ConcentrationLimitShape a sh:NodeShape ;")
    lines.append('    sh:targetClass cosmo:CosmeticIngredient ;')
    lines.append('    sh:sparql [')
    lines.append('        a sh:SPARQLConstraint ;')
    lines.append('        sh:message "EU concentration limit exceeded: {?this} has {?actual} but limit is {?limit}" ;')
    lines.append('        sh:select "PREFIX cosmo: <http://example.org/cosmo#> PREFIX xsd: <http://www.w3.org/2001/XMLSchema#> SELECT $this ?actual ?limit WHERE { $this cosmo:actualConcentration ?actual . $this cosmo:maxConcentrationEU ?limit . FILTER(xsd:double(?actual) > xsd:double(?limit)) }" ;')
    lines.append('    ] .')
